2024-0308-03)자료 검색명령
 ...
 (사용형식)
  ...
  [HAVING 조건]
   ...
   - ORDER BY 절
     . 출력 데이터의 정렬 지정
     . 기술된 '컬럼명'이나 '컬럼의인덱스(SELECT 절에 기술된 컬럼의 순번)'를 기준으로
       오름차순(ASC)이나 내림차순(DESC)으로 정렬하며
     . 두 번째 기술되는 컬럼명은 첫 번째 컬럼명으로 정렬을 할 수 없을 때(같은 값)
       적용된다.
       
 사용 예) 회원테이블(MEMBER)에서 회원번호(MEM_ID), 회원명(MEM_NAME), 직업(MEM_JOB),
 ...
 사용 예) 상품분류테이블(LPROD)의 모든 자료를 조회하시오
         SELECT * 
         FROM LPROD;
 
 사용 예) 상품테이블(PROD)에서 상품번호(PROD_ID), 상품명(PROD_NAME), 
 ...
 사용 예) 사원테이블(HR.EMPLOYEES)에서 사용하고 있는 부서번호(DEPAERMENT_ID)를 모두 
         조회하되 중복하지 않게 조회하며 순서대로 출력하시오(작은값->큰값)
         SELECT DISTINCT DEPARTMENT_ID AS 부서번호
         FROM HR.EMPLOYEES ORDER BY DEPARTMENT_ID ASC;

 사용 예) HR계정의 부서테이블의 모든 자료를 조회하시오.
        SELECT DEPARTMENT_ID
         FROM HR.DEPARTMENTS;
         
  사용예) 회원테이블에서 회원번호, 회원명, 마일리지를 조회하되 마일리지가 맣은 회원부터 출력하시오.
        SELECT MEM_ID AS 회원번호,
                MEM_NAME AS 회원명,
                MEM_MILEAGE AS "보유 마일리지"
                FROM MEMBER ORDER BY MEM_MILEAGE DESC;

  사용예) 사원테이블(HR.EMPLOYEES)에서 사원번호(EMPLOYEE_ID), 사원명(EMP_NAME), 부서코드(DEPARTMENT_ID), 급여(SALARY)를
