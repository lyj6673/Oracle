2024-0329-01)SEQUENCE 객체
 ...
 CREATE SEQUENCE 시퀀스명
    ...
    [MAXVALUE 값|NOMAXVALUE] - 최대 값 NOXAMVALUE가 default이며 10^27
    [MINVALUE 값|NOMINVALUE] - 최소 값 NOMINVALUE가 default이며 1임
    [CYCLE | NOCYCLE] - default는 NOCYCLE이며 최대[최소]까지 도달 후
                        다시 시퀀스 생성 여부 결정
 ...
 **** 시퀀스가 생성된 후 반드시 NEXTVAL부터 호출되어야 한다
      NEXTVAL이 호출될 때마다 증감값만큼 증감됨(주의)

사용 예)분류테이블에 다음 자료를 입력하시오
-----------------------------------
  컬럼             값
-----------------------------------
LPROD_GU    10부터 증가되는 값(시퀀스 이용)
LPROD_GU    'P501'
LPROD_NM    '농산물'
