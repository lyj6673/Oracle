2024-0404-01)Stored Procedure(procedure)
 - 미리 컴파일되어 서버에 저장된 모듈
 - 반환값이 없는 서브프로그램
 - 프로시져 캐쉬에 저장되어 호출 실행되므로 처리속도가 빠름
 - DB내부 구조 보안
 (사용형식)
...
[실행]
 - 반환 값이 없으므로 SELECT문 등 표준 SQL문에는 사용할 수 없음
 (실행문 형식-1:독립적으로 실행)
  EXEC|EXECUTE 프로시저명[(매개변수list)];
  
 (실행문 형식-2:다른 블록이나 함수, 프로시져에서 실행)
  프로시저명[(매개변수list)];
  
사용 예)키보드로 기간(년월)을 입력 받아 해당 기간동안 상품별 매입현황을 조회하시오
 ...
 [실행]
 EXECUTE proc_buyprod_01('202001','202002');
 
 사용예) 부서번호를 입력 받아 해당 부서의 평균급여와 부서명, 관리사원명을 출력하면
        프로시져를 작성하시오 --문제가 없는데 실행이 안됨
...
[실행]- 키보드로 부서번호 입력
  ...
  BEGIN
    proc_emp_01(L_DID,L_ASAL,L_DNAME,L_ENAME);
...
사용 예)회원아이디를 입력받아 이름, 직업, 마일리지를 TEMP_MEMBER테이블에
       저장하는 프로시져를 작성하시오
...
사용 예)오늘(2024/04/04) 'f001'회원이 'P201000017'상품을 3개 구매했다
       이를 처리하는 프로시져를 작성하시오
  1) CART테이블에 저장
  2) REMAIN 테이블에 재고 수량 UPDATE
  3) 'f001'회원의 마일리지 UPDATE
  
  CREATE OR REPLACE PROCEDURE proc_cart_insert(
    P_DATE DATE, P_MID IN MEMBER.MEM_ID%TYPE,
    P_PID IN PROD.PROD_ID%TYPE, P_QTY NUMBER)
    
  IS
    L_CART_NO CHAR(13):=TO_CHAR(P_DATE,'YYYYMMDD');
    L_MILEAGE NUMBER:=0;
    L_REC_CNT NUMBER:=0; --CART 테이블의 오늘 날짜 행의 수
  BEGIN
    --카트번호(CART_NO)생성
