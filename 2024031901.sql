...
2. NVL(col, value)
  ...
  - 'col'과 'value'는 반드시 같은 타입 이어야 함
  
  사용 예)사원테이블에서 50-90번 부서에 속한 사원들의 보너스를 계산하여 출력하시오
         보너스 = 급여*영업실적코드이며 보너스가 없으면 '보너스 없음'을 출력하시오
         Alias는 사원번호, 부서번호, 급여, 영업실적코드, 보너스
         부서번호 순으로 출력
    ...
    SELECT EMPLOYEE_ID AS 사원번호,
           ...
           NVL(TO_CHAR(SALARY*COMMISSION_PCT, '99,999.9'),'보너스없음') AS 보너스
       FROM HR.EMPLOYEES
      WHERE DEPARTMENT_ID BETWEEN 50 AND 90
      ORDER BY 2;
      
사용 예)회원테이블에서 마일리지가 NULL인 회원을 찾아 비고난에 '휴먼회원'을, NULL이
        아닌 회원은 해당 마일리지를 출력하시오
        Alias는 회원번호, 회원명, 비고
        SELECT MEM_ID AS 회원번호,
               MEM_NAME AS 회원명,
              NVL(TO_CHAR(MEM_MILEAGE,'9,999'), '휴먼회원') AS 비고
          FROM MEMBER;
        
 사용 예)다음은 2020년 5월 회원별 구매실적에 따른 마일리지를 계산한 쿼리이다.
        마일리지가 없는 상품의 마일리지는 0으로 계산할 것
...
3. NVL2(col, value1, value2)
  - 'col' 값이 NULL이면 'value2'를 반환하고 NULL이 아니면 'value1'값을 반환
  - 'value1'과 'value2'의 데이터 타입은 일치해야 함
  - NVL의 기능을 포함
  
 사용 예)상품테이블에서 색상정보(PROD_COLOR)를 조회하여 NULL이면 '색상 없는 상품',
...
4. NULLIF(col1, col2)
  ...
  **상품테이블에서 분류코드 'P301'에 속한 상품의 매출가격을 매입가격으로 
    조정하시오.
 ...
 사용 예)상품테이블에서 매입가격과 매출가격이 동일한 품목은 비고난에 '단종예정상품'을
        출력하시오.
    ...
    SELECT PROD_ID AS 상품코드,
           PROD_NAME AS 상품명,
           PROD_COST AS 매입가,
           NVL(LPAD(TO_CHAR(NULLIF(PROD_COST,PROD_PRICE)),18),'단종예정상품') AS 매출가
        FROM PROD;
  ...
  ** 상품테이블에서 각 상품의 마일리지(PROD_MILEAGE)를 다음 조건에 맞게 계산하여
     입력하시오
     상품의 마일리지(PROD_MILEAGE)=상품의 이익(판매가-매입가)의 1%
     단, 상품의 마일리지가 100미만이면 NULL처리하시오.
