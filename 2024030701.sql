2024-0307-01)오라클 데이터 타입
  ...
  . 문자열 자료를 저장하기 위한 데이터 타입은 고정형과 가변형으로 구분
  . 고정길이 문자열 : CHAR
...
사용 예)
  ...
  INSERT INTO TEMP01(COL1,COL2,COL3) VALUES('대전시 중구 846', '대전시 중구 계룡로 846','대전시 중구');
  ...
  2) VARCHAR2
    ...
    - 한글 한 글자는 3BYTE가 필요
    - VARCHAR, NVARCHAR2,NVARCHAR와 기능은 동일
    - NVARCHAR2, HVARCHAR는 UTF-8, UTF-16 형식으로 데이터를 변환하여 저장(국제 표준형식)
    (사용형식)
    ...
    INSERT INTO TEMP02 VALUES('IL POSTONO BOYHOOD','IL POSTONO BOYHOOD','IL POSTONO BOYHOOD',
                              'IL POSTONO BOYHOOD');
  ...
  3) LONG
   ...
   - 데이터가 저장되고 남는 공간은 시스템에 반환 됨(공간 부족 시 에러)
   - 한글 한 글자는 3BYTE가 필요
   - 한 테이블에 하나의 long타입 컬럼만 정의 할 수 있음
   - 기능 개선 서비스가 중단됨=>CLOB(Character Large Obdect)로 기능이 개선되어 제공
   - 일부 기능(함수)은 사용할 수 없음
   (사용 형식)
     컬럼명 LONG
 ...
 --     COL2 LONG,
  ...
  사용 예)
    ...
    SELECT MOD ((TO_DATE('20000915') - TO_DATE('00010101'))-1,7) FROM DUAL;
