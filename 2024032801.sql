2024-0328-01)VIEW
 ...
 - VIEW를 사용하는 경우
  . 필요한 정보가 복수 개의 테이블에 분산되어 있는 경우
  . 특정 자료에 대한 접근을 제한해야 하는 경우
...
(사용 형식)
    ...
    . '컬럼list' : 뷰에서 사용할 컬럼명들. 생략하면 SELECT문에 사용된 컬럼의 별칭이
      뷰의 컬럼명이 되며,  SELECT문에서 별칭을 사용하지 않은 경우 SELECT문의
      컬럼명이 뷰의 컬럼명이 됨
    . WITH CHECK OPTION : SELECT 문의 WHERE 절을 위배하는
      DML명령을 뷰에 적용할 수 없다(원본 테이블은 WITH CHECK OPTION에 상관없이
      DML명령을 수행할 수 있다)
    . WITH READ ONLY :읽기 전용 뷰를 생성함
    . WITH READ ONLY와 WITH CHECK OPTION은 같이 사용할 수 없다.

사용 예)회원테이블에서 마일리지가 3000이상인 회원들의 회원번호, 회원명, 마일리지로
       뷰를 생성하시오.
...
사용 예) 뷰  V_MEM_MILEAGE에서 'b001'회원의 마일리지를 2960으로
        변경하시오
  UPDATE  V_MEM_MILEAGE
     SET 마일리지=2960
   WHERE 회원번호='b001';
   -- WITH CHECK OPTION이 사용되어 업데이트 안됨--
