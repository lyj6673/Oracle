2024-0401-01)PL/SQL
 ...
 - block구조로 여러 SQL문을 한 번에 실행 가능
 - 모듈화/캡슐화가 가능
 - 서버에 저장되어 빠른 실행과 반복 사용 가능
 ...
 1.익명 블록
  ...
  BEGIN
    ...
    END;
 사용 예)2020년 5월 가장 많이 구매한 고객의 회원번호, 이름, 주소, 구매액을
        출력하는 익명블록 작성
  DECLARE
 ...
 ** 오라클 변수의 종류와 선언 형식
 ...
 (2) REFERENCE 변수
  ...
  . (사용형식)
     ...
     식별자 [CONSTANT] 테이블명.컬럼명%ROWTYPE [:=초기값] --ROW(행) 참조
     
     (ex) L_MILEAGE MEMBER.MEM_MILEAGE%TYPE --L_MILEAGE변수의 타입이
                                            --MEMBER테이블의 MEM_MILEAGE컬럼의 타입과
                                            --크기가 같게 선언
          L_CART_REC CART%ROWTYPE; --L_CART_REC 변수는 CART테이블 한 행과 같은 타입으로
                      --선언됨. 따라서 L_CART_REC로 참조할 수 있는 값의
                      --형태는 CART_MEMBER에 저장된 자료는 L_CART_REC.CART_MEMBER로
                           --CART_PROD는 L_CART_REC.CART_NO로 각각 참조 가능함
  (3)BIND 변수
   . 프로시져와 함수에서 사용하여 매개변수의 전달을 담당하는 변수
   (ex) CREATE OR REPLACE FUNCTION fn_create_cart_no(
            P_DATE IN DATE,
...
사용 예)회원들의 마일리지를 조회하여 평균값보다 큰 마일리지를 보유한 회원은
       '우수회원 입니다'라는 메시지를, 평균보다 적으면 '분발하세요'라는
       메시지를 출력하시오
       ----------------------------------------------------
       회원번호      회원명      보유마일리지      비고
       ----------------------------------------------------
 ...
 BEGIN
   ...
   SELECT AVG(MEM_MILEAGE) INTO  L_AMILE
     ...
     DBMS_OUTPUT.PUT_LINE('회원번호      회원명      보유마일리지      비고');
     DBMS_OUTPUT.PUT_LINE('------------------------------------------');
 ...
 사용 예)오늘이 2020년 7월 28일이라고 가정하고 CART_NO를 생성하는 
        함수를 작성하시오
 ...
 (실행)
 INSERT INTO CART VALUES('H001',fn_create_cartno(SYSDATE),
                         'P201000012',10);
...
** 커서
 ...
 가)묵시적 커서
  ...
  . 결과가 출력될 때 OPEN되었다가 출력이 종료되면 즉시 CLOSE됨
 ...
 (사용형식)
    CURSOR 커서명[(변수list)] IS
        SELECT 문;
    . '변수list' : 커서 OPEN문에서 전달되는 값을 보관하는 바인딩 변수로 크기를
       선언하지 않음
    . 커서의 사용 절차
      A)커서선언(선언부) -> 커서OPEN -> 커서 FETCH -> 커서 종료(CLOSE)
    . 커서OPEN
      - 사용할 커서를 OPEN
      - (기술형식)
        OPEN 커서명[(변수list)]
...
사용 예) 키보드로 10-110 사이의 부서번호를 입력 받아 해당 부서에 근무하는 사원
    ...
    BEGIN
        ...
        DBMS_OUTPUT.PUT_LINE(' 부서번호 :' || '&P_DEPT');
        LOOP
            FETCH cur_emp_dept INTO L_EID, L_ENAME, L_HDATE, L_SAL;
...
사용 예) 상품테이블에서 분류코드 'P201'에 속한 상품 중 판매가가 가장 비싼 상품과
        가장 저렴한 상품을 조회하시오. 상품번호,상품명,판매가
  SELECT B.PROD_NAME AS BNAME,
         B.PROD_ID AS BPID,
         B.PROD_PRICE AS BPRICE
    FROM (SELECT MAX(PROD_PRICE) AS M1,
                 MIN(PROD_PRICE) AS M2
            FROM PROD
           WHERE PROD_LGU='P201')A, PROD B
    WHERE B.PROD_LGU='P201'
      AND B.PROD_PRICE IN(A.M1,A.M2);

사용 예)거주지가 '충남'인 회원들을 출력하시오. 회원번호, 회원명, 주소
    DECLARE --실행안됨 ~
  ...
  (3)FOR 문
   ...
   FOR 레코드명 IN 커서명 | 커서용 SELECT문 LOOP
     반복처리문
        :
   END LOOP;
   ** FOR문으로 커서를 사용하는 경우 OPEN/FETCH/CLOSE문이 생략 됨
      선언부에서 선언된 커서를 사용하거나(커서명) 또는 커서 구성에
      사용되는 SELECT문을 직접 기술할 수 있다
    
   ** FOR문을 사용할 경우 커서 내의 컬럼 참조는
      레코드명.컬럼명으로 참조함
