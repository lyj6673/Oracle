...
1. IS(IS NOT)
  ...
  사용 예)상품테이블에서 크기(PROD_SIZE)컬럼에 데이터가 없는 상품을 조회하시오
...
2. IN 연산자
    (사용형식)
     ...
     . OR 연산자로 대신할 수 있음
     . ( )안에 서브쿼리가 올수 있음
     . 값1,값2,...,값n 에 포함되지 않은 경우를 판단할 때는 NOT IN(값1,값2,...,값n)사용
     . 주로 불연속적이거나 비규칙적인 값들에 대한 OR연산이 필요할 때 사용
     
사용 예) HR계정의 부서테이블에서 20,50,60,90,110번 부서의 부서코드, 부서명, 관리사원번호를
        조회하시오.
...
[IN 연산자]
 ...
 3.ANY(SOME) 연산자
  (사용형식)
   ...
   . '컬럼명'의 값이 ANY다음 ( )안의 값 중 어느 하나와 기술된 '관계연산자'의
     조건을 만족하면 결과가 참이 되는 연산자
   . ANY와 SOME은 동일함
   . ANY 앞에 관계연산자가 '='인 경우가 IN연산자
   
 사용 예)사원테이블에서 20번 부서의 최소급여, 40번 부서의 최소급여, 70번 부서의 최소급여보다
        더 적은 급여의 사원을 조회하시오
        Alias는 사원번호, 사원명, 부서번호, 급여
        (20번 부서의 최소급여, 40번 부서의 최소급여, 70번 부서의 최소급여)
        6000                    6500                 10000
    SELECT EMPLOTEE_ID AS 사원번호,
           EMP_NAME AS 사원명,
 ...
 사용 예)대전 거주 회원 중 충남에 거주하는 회원들의 가장 적은 마일리지보다 더 많은 마일리지를
...
4. ALL연산자
 ...
 사용 예)대전 거주 회원 중 충남에 거주하는 회원들의 가장 큰 마일리지보다 더 많은 마일리지를
        보유한 회원의 회원번호, 회원명, 마일리지를 조회하시오.
...
5. LIEK 연산자
 ...
 . EX) '김%' -> '김'으로 시작하는 모든 문자열은 모두 허용됨(NULL 포함)
       '%김' -> '김'으로 끝나는 모든 문자열은 모두 허용
       '%김%'-> 단어 중 '김'글자가 포함된 모든 문자열은 모두 허용
       
 - '-'
 . '-'가 사용된 이후 하나의 문자열과 대응됨
 . EX) '김-' -> '김'으로 시작하고 2글자인 문자열과 대응
       '-김' -> '김'으로 끝나는 2글자의 문자열과 대응
       '-김-'-> 3글자로 구성된 단어 중 '김'글자가 포함된 모든 문자열은 모두 허용
 - LIKE 연산자는 문자열 비교에 사용되며, 많은 결과를 산출하기 때문에
   쿼리의 효율성은 낮은편임
 - 문자열이외의 자료(날짜, 숫자)에 사용은 자제할 것
 
 사용 예) 회원테이블에서 거주지가 대전인 회원들의 이름, 주소, 직업, 마일리지를 조회하시오.
 ...
 사용 예)장바구니에서 2020년 5월 판매 정보를 조회하시오
        Alias는 회원번호, 제품번호, 구매수량
 ...
 사용 예)매입테이블에서 2020년 2월 매입정보를 조회하시오.
...
사용 예)매입테이블(BUYPROD)에서 2020년 3월 1일부터 3월 15일까지 매입정보,
       매입일자(BUY_DATE), 매입상품(BUY_PROD), 매입수량(BUY_QTY)을 
       일자순으로 조회하시오.
    ...
    SELECT BUY_DATE AS 매입일자,
           ...
           BUY_QTY AS 매입수량
      FROM BUYPROD
 --    WHERE BUY_DATE>=TO_DATE('20200301') AND BUY_DATE<=TO_DATE('20200331')
     WHERE BUY_DATE BETWEEN TO_DATE('20200301') AND TO_DATE('20200315')
     ORDER BY 1; 

사용 예)매출테이블에서 2020년 6~7월에 판매된 상품 중 분류코드 'P100'~
       'P200'에 속한 상품들의 매출정보(날짜, 상품코드, 매출수량)를
       날짜, 분류코드 순으로 조회하시오.
