...
1. SYSDATE
  ...
  사용 예)회원테이블의 생년월일 자료를 이용하여 이번주와 다음주에 생일인 회원
         들에게 문자메시지를 보내려고 한다. 해당 회원들을 조회하시오.
    ...
    SELECT MEM_ID AS 회원번호,
     ...
     WHERE EXTRACT(MONTH FROM SYSDATE)=EXTRACT(MONTH FROM MEM_BIR)
       AND EXTRACT(DAY FROM SYSDATE) BETWEEN EXTRACT(DAY FROM SYSDATE)
           AND EXTRACT(DAY FROM SYSDATE+7);

2. ADD_MONTHS(d1, n) 
  - 시스템시간 반환, 주어진 날짜에 ADD한 만큼 반환
  
  사용 예)HR계정의 사원테이블에서 입사일(HIRE_DATE)을 15년을 더한 날짜로 변경하시오.
    (UPDATE 문의 사용형식)
    UPDATE 테이블명
       SET 컬럼명=값[,]
          [컬럼명=값[,]
    ...
    UPDATE HR.EMPLOYEES
       SET HIRE_DATE=ADD_MONTHS(HIRE_DATE, 100);
...
3. NEXT_DAT(d1, fmt)
 - 주어진 날짜 d1이후 처음 만나는 'tmt'요일의 날짜를 반환
  - fmt : '월요일', '월', '화요일',...등의 요일명
  
사용 예) SELECT NEXT_DAY(SYSDATE,'월요일'),
               NEXT_DAY(SYSDATE,'금') FROM DUAL;
               
4. LAST_DAY 
 - 주어진 날짜 d1에 포함된 월의 마지막날짜 반환
 - 주로 2월의 마지막일자나 사용자가 실행 중 입력 받은 월의 마지막 일자를 구할 때 사용
 
 사용 예)매입테이블에서 2020년 2월 매입건수를 구하시오
    SELECT COUNT(*) AS 매입건수
        FROM BUYPROD
       WHERE BUY_DATE BETWEEN TO_DATE('20200201') AND 
                              LAST_DAY(TO_DATE('20200201'));
                              
사용 예) 키보드로 월을 입력 받아 해당 월의 전체 매출금액 합계를 구하시오.
...
5. MONTHS_BETWEEN(d1, d2) 
 ...
 사용 예)사원테이블에서 근속년수를 구하시오. 근속년수는 월까지 고려하여 구하시오.
...
6. EXTRACT() 
 ...
 사용 예) 사원테이블에서 각 월별 입사인원수를 조회하시오.
 ...
 사용 예) 매입테이블에서 2020년 상반기 월별 매입집계를 조회하시오
